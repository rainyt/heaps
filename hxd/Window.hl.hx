...
class Window {
	...
	**/
	...
	function getPixelRatio():Float {
		...
		#if highdpi
		#if (ios || mac)
		return zygame.utils.hl.NativeTools.get_pixel_ratio();
		#else
		return 1;
		#end
		#else
			...
			switch (mouseMode) {
				case Relative(_, restorePos) | AbsoluteUnbound(restorePos):
					if (restorePos) {
						...
						curMouseX = hxd.Math.iclamp(Std.int(curMouseX), 0, width);
						curMouseY = hxd.Math.iclamp(Std.int(curMouseY), 0, height);
					}
					#if hldx
					window.setCursorPosition(Std.int(curMouseX), Std.int(curMouseY));
					#elseif hlsdl
					window.warpMouse(Std.int(curMouseX), Std.int(curMouseY));
					#end
				default:
			}
		}
			...
			case MouseMove if (!hxd.System.getValue(IsTouch)):
				switch (mouseMode) {
					...
					case Relative(callback, _):
						#if (hldx || hlsdl)
						var ev = new Event(EMove, e.mouseXRel * getPixelRatio(), e.mouseYRel * getPixelRatio());
						#else
						var ev = new Event(EMove, e.mouseX * getPixelRatio() - curMouseX, e.mouseY * getPixelRatio() - curMouseY);
						#end
						callback(ev);
					...
					case AbsoluteUnbound(_):
						#if (hldx || hlsdl)
						curMouseX += e.mouseXRel * getPixelRatio();
						curMouseY += e.mouseYRel * getPixelRatio();
						#else
						curMouseX += e.mouseX * getPixelRatio() - curMouseX;
						curMouseY += e.mouseY * getPixelRatio() - curMouseY;
						#end
						eh = new Event(EMove, curMouseX, curMouseY);
				}
			case MouseWheel:
	...
	public static function getMonitors():Array<Monitor> {
		return [
			for (m in #if hldx dx.Window.getMonitors() #elseif hlsdl sdl.Sdl.getDisplays() #else [] #end) {name: m.name, width: m.right - m.left,
				height: m.bottom
				- m.top}
		];
	}
